...
| 图像分类 | 识别图片类别 | [迷你 CNN 图像分类](examples/tiny_cnn_image_classification.py) |
...
- 理解图像分类的输入、特征提取和分类输出。
- 在学习完整 CNN 框架前建立直觉。
- 对照卷积演示，理解“卷积特征可以作为分类器输入”。

说明：

当前示例使用 5x5 二值小图像和标准库实现，不追求真实图片精度。它的价值是把 CNN 的核心流程拆小：图像输入、卷积特征、分类概率。
...
如果准备进入 NumPy、scikit-learn 或 PyTorch，可以继续看 [神经网络框架学习入口](FRAMEWORK_ROADMAP.md)。
